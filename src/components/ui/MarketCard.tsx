  ...
  const renderBinaryOdds = () => (
    <div className={styles.oddsContainer}>
      <div className={styles.oddsOption}>
        ...
        <span className={`${styles.oddsValue} ${styles.yes}`}>{market.yesOdds}%</span>
      </div>
      <div className={styles.oddsOption}>
        <span className={styles.oddsLabel}>NO</span>
        <span className={`${styles.oddsValue} ${styles.no}`}>{market.noOdds}%</span>
      </div>
    </div>
  );
  ...
  const renderMultiChoiceOdds = () => (
    <div className={styles.multiChoiceContainer}>
      ...
      {market.options && market.options.length > 3 && (
        <div className={styles.moreOptions}>+{market.options.length - 3} more options</div>
      )}
    </div>
  );
    ...
    >
      ...
      <div className={styles.footer}>
        ...
        <div className={styles.status}>
          <div
            className={`${styles.statusDot} ${market.isActive ? styles.active : styles.inactive}`}
          ></div>
          <span className={styles.statusText}>{market.isActive ? "Active" : "Resolved"}</span>
        </div>
      </div>
    </div>
  );
