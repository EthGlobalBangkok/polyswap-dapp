  ...
  if (isLoading) {
    return (
      <div className={styles.stepContent}>
        ...
        <p className={styles.stepDescription}>Generating transaction data for your wallet...</p>
  ...
  if (isWaiting) {
    ...
    const setupStepInfo = isSetupTransaction
      ? batchData?.needsFallbackHandler
        ? "1/2 - Setting up handler"
        : "2/2 - Setting up verifier"
      : null;
    ...
    <div className={styles.stepContent}>
      {batchData?.setupOnlyBatch ? (
        <>
          ...
          <p className={styles.stepDescription}>
            Your Safe wallet needs to be configured before creating orders. This is a one-time setup
            process.
          </p>
          ...
          >
            ...
            <p style={{ color: "#1565c0", fontWeight: "bold", marginBottom: "8px" }}>
              ⏳ After signing this transaction and waiting for confirmation, please{" "}
              <strong>create your order again</strong> to continue setup.
            </p>
            {batchData.needsDomainVerifier && !batchData.needsFallbackHandler && (
              <p style={{ color: "#1565c0", fontSize: "0.9em" }}>
                ℹ️ After this step completes, your Safe will be fully configured and you can create
                orders normally.
              </p>
            )}
          </div>
          ...
          >
            {isSending
              ? "Signing..."
              : `Sign Setup Transaction ${batchData.needsFallbackHandler ? "(1/2)" : "(2/2)"}`}
          </button>
        </>
      ) : (
        <>
          <h2 className={styles.stepTitle}>
            {batchData?.needsApproval
              ? "Sign Batch Transaction (with Approval)"
              : "Sign Transaction"}
          </h2>
          <p className={styles.stepDescription}>
            Your Polymarket order has been created. Now you need to sign and broadcast your PolySwap
            conditional order using your Safe wallet.
          </p>
          ...
          >
            ...
            {batchData?.needsApproval && (
              <p>
                • <strong>Each transaction in the batch requires individual signing</strong>
              </p>
            )}
          </div>
          ...
          <div className={styles.infoBox}>
            ...
            <div className={styles.transactionDetails}>
              <div className={styles.detailRow}>
                ...
                <span className={styles.detailValue}>
                  {batchData?.needsApproval
                    ? `Batch (${transactionSummary?.transactionCount} transactions)`
                    : "Single Transaction"}
                </span>
              </div>
              <div className={styles.detailRow}>
                ...
                <span className={styles.detailValue}>
                  {polymarketOrderHash.substring(0, 10)}...
                  {polymarketOrderHash.substring(polymarketOrderHash.length - 8)}
                </span>
              </div>
              {batchData?.needsApproval && (
                <div className={styles.detailRow}>
