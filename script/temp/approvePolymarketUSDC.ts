...
async function approveUSDC() {
  try {
    ...
    const CTF_EXCHANGE = process.env.POLYMARKET_CONTRACT_ADDRESS || '0x4bFb41d5B3570DeFd03C39a9A4D8dE6Bd8B8982E'; // CTFExchange
    ...
    for (const contract of contracts) {
      ...
      if (currentAllowance > 0n) {
        console.log(`   ✅ ${contract.name} already has allowance: ${ethers.formatUnits(currentAllowance, decimals)} ${symbol}`);
        continue;
      }
    ...
    for (const contract of contracts) {
      ...
      console.log(`   ${contract.name}: ${isMax ? '♾️  UNLIMITED' : ethers.formatUnits(allowance, decimals)} ${symbol}`);
    }
